...
export const getScruitnize = async (payload: any,currentPage:number) => {
  try {
    ...
    const apiUrl = `${BASE_URL}/gateway/officer/inspection/getlistofrecordsassignedbydoreg/${currentPage}`
    ...
    console.log("Scruitnize result:", data)
    return data
  } catch (error) {
    console.error("Error in Scruitnize:", error)
    throw error
  }
}
