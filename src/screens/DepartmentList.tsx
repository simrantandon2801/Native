...
import {GetUserDept} from '../database/Users';
        ...
        <View
          ...
          editingDepartment.department_id === department.department_id ? (
            ...
            <>
              ...
              {department.children?.length > 0 && (
                <TouchableOpacity
                  onPress={() => toggleExpand(department.department_id)}>
                  <Ionicons
              ...
              {/* Department Name */}
              <Text style={[styles.cell, {fontWeight: 'bold', flex: 1}]}>
              ...
              <Text style={[styles.cell, {flex: 1, textAlign: 'right'}]}>
                {department.department_head
                  ? `${
                      users.find(
                        user => user.user_id === department.department_head,
                      )?.first_name
                    } ${
                      users.find(
                        user => user.user_id === department.department_head,
          ...
          <View style={{marginLeft: 20, position: 'relative'}}>
            {department.children.map(child => (
              <View
                ...
                {renderHierarchyLines(level + 1, true)}{' '}
                {/* Horizontal line for child */}
        ...
        <View style={styles.leftPanel}>
          ...
          <FlatList
            ...
            renderItem={({item, index}) => {
              const isExpanded =
                headingDepartment &&
                headingDepartment.department_id === item.department_id;
              return (
                <>
                  <TouchableOpacity
                    style={styles.row}
                    ...
                    {editingDepartment &&
                    editingDepartment.department_id === item.department_id ? (
                      <TextInput
                        style={styles.editInput}
                        value={newDepartmentDetails.department_name}
                        onChangeText={text =>
                    ...
                    editingDepartment.department_id === item.department_id ? (
                      ...
                      <Text style={styles.cell}>
                        {users.find(
                          user => user.user_id === item.department_head,
                  ...
                  {isExpanded &&
                    ...
                    subDepartments.length > 0 && (
                      <View style={styles.subDepartmentsContainer}>
                        <Text style={styles.subDeptHeader}>
                          Sub-Departments
                        </Text>
                        {subDepartments.map((subDept, subIndex) => (
                          <View key={subDept.department_id} style={styles.row}>
                            ...
                            <Text style={styles.cell}>
                              {users.find(
                                user =>
                                  user.user_id === subDept.department_head,
                              )?.first_name || 'Not Assigned'}
                            </Text>
