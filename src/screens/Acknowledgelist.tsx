...
const AcknowledgeList: React.FC = () => {
  ...
  const fetchAcknowledgement = useCallback(
    async (page: number) => {
      setIsLoading(true)
      setError(null)
      try {
  ...
  useEffect(() => {
    ...
    // We'll only fetch when search button is clicked
  }, [userId, currentPage])
  ...
  }, [userId, currentPage, fetchAcknowledgement, hasSearched])
