...
import { getAcknowledgedInspectionCount, InspectionPayload } from '../database/Dashboardapi';
...
export default function DashboardScreen() {
  ...
  const fetchloadData = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert('Error', `Failed to load inspection data: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const renderInspectionItem = ({ title, count, iconName, isOnline }: InspectionItem) => (
    <View style={styles.item}>
      ...
      <View style={styles.onlineStatus}>
        <View
          style={[styles.statusDot, { backgroundColor: isOnline ? '#01DB31' : '#DE9300' }]}
        />
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.grid}>
          {inspectionItems.map((item, index) => renderInspectionItem(item))}
        </View>
        <Text style={styles.listTitle}>Acknowledged Inspections</Text>
        {acknowledgedData.paginationListRecords.length > 0 ? (
          acknowledgedData.paginationListRecords.map((item, index) => (
            <View key={`${item.displayRefId || 'item'}-${index}`} style={styles.listItem}>
