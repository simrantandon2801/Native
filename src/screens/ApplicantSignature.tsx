   ...
   const handleViewImage = async (documentPath: string) => {
    ...
    const handleApplicantContactChange = (text: string) => {
      ...
      if (numericValue.length <= 10) {
        setApplicantContact(numericValue); // Update state only if length is <= 10
      }
    
      // Validate the input length and set an error if necessary
      if (numericValue.length !== 10) {
...
>
  ...
  <TextInput
    ...
    onChangeText={handleApplicantContactChange} // Use the custom handler
